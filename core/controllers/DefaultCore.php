...
class DefaultCoreController extends CoreController{
	...
	private static function copy($source, $destination, $action, $log=false){
					...
					if (is_file($source)){
						...
						if (!$r){
							$m = 'La manipulation a échouée';
							if ($log) $m.=' ('.$source.')';
							$result = array('error'=>1, 'msg'=>$m);
						}
						return $result;
					}
					if (is_dir($source)){
						...
						while ($f = readdir($dir)) {	
							if($f!="." && $f!="..") {								
								$r = self::copy($source.'/'.$f, $destination.'/'.$f, $action, $log);
								if ( $r['error']==1 ) return $r;
								self::$i++;
								if ($log){;
									self::setLog($log['action'], round(100*self::$i/$log['nb']), $log['file'], $source);
								}
							}
						}
						if ($action!=='copy') {
